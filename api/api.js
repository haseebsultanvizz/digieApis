...
router.post('/editAutoOrder', async(req, resp) => {
        ...
        let sell_order_id = buyOrderReocrd['sell_order_id'];

        console.log("sell_order_id here");
        console.log(sell_order_id);
...
async function listOrderListing(postDAta, dbConnection) {
    ...
    if (postDAta.status == 'all') {
        ...
        var orderArr = returnArr.slice().sort((a, b) => a.created_date - b.created_date)
    } else {
...
/* To get Buy order by order ID By Ali 11-27-2019 */
function getBuyOrder(order_id) {
    return new Promise((resolve) => {
        conn.then((db) => {
            let where = {};
            where['_id'] = order_id;
            db.collection('buy_orders').find(where).limit(1).skip(skip).toArray((err, result) => {
                if (err) {
                    console.log(err)
                } else {
                    resolve(result)
